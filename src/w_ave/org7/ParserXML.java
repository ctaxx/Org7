...
*/
	private void workWithNodes(NodeList nodeList, ArrayList<Item> arrayList, Item ancestor){
		for(int i= 0; i<nodeList.getLength(); i++){
			...
			if ((node.getNodeType() == Node.ELEMENT_NODE)&(!node.getNodeName().equals("timestamp") )){
				...
				/**
				 * создание элемента, пересыпание данных,
				 * св€зывание с предком и с узлами DOM
				 */
	...
	public void saveListToXML(){
		try {
			...
			StreamResult streamResult = new StreamResult(new File("/sdcard/Org7context.txt"));
